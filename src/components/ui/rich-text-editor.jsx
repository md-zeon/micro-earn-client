...
function showFakeSelection(editor) {
...
function clearFakeSelection(editor) {
  editor.view.dispatch(
...
const contentClassName = cn(
  ...
  "[&_.ProseMirror_mark]:rounded-sm [&_.ProseMirror_mark]:bg-amber-500/40 [&_.ProseMirror_mark]:px-0.5 [&_.ProseMirror_mark]:text-foreground",
...
function RichTextEditorContent({ className, ...props }) {
...
function RichTextEditorToolbar({ className, children, ...props }) {
  return (
    <TooltipProvider>
      <div
...
function RichTextEditorSeparator({ className, ...props }) {
  return (
    <Separator
...
function RichTextEditorLinkBubble() {
  ...
  const [, reposition] = React.useReducer((n) => n + 1, 0);
  ...
  const linkFromNode = React.useCallback((node) => {
    const el = node instanceof HTMLElement ? node : node?.parentElement;
    return el?.closest("a") ?? null;
  }, []);

  const linkAtSelection = React.useCallback(() => {
  ...
  const show = React.useCallback((el) => {
  ...
  React.useEffect(() => {
    ...
    const onOver = (e) => {
      ...
      const el = linkFromNode(e.target);
      if (el) {
        clearHide();
        show(el);
      }
